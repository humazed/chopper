...
class ChopperGenerator extends GeneratorForAnnotation<chopper.ChopperApi> {
  ...
  Method _generateMethod(MethodElement m, String baseUrl) {
    ...
    return new Method((b) {
      ...
      if (hasParts) {
        blocks.add(_generateList(parts, fileFields)
            .assignFinal(_partsVar, refer('List<PartValue>'))
            .statement);
        blocks.add(
  ...
  Expression _generateList(
    ...
    fileFields.forEach((p, ConstantReader r) {
      ...
      list.add(
        Code(
            "${p.displayName} == null ? null : PartFile<${p.type.displayName}>('$name', ${p.displayName})"),
      );
    });
